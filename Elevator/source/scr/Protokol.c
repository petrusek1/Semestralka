/**
 * \file serial.c
 *
 * \brief elevator serial interface
 */

#include "stdio.h"
#include "string.h"

#include "include/LPSCI.h"
#include "include/Components.h"
#include "include/Definitions.h"
#include "include/Protokol.h"

volatile char ack =0;
extern volatile uint8_t rx_buff_write_pointer;
extern volatile uint8_t rx_buff_read_pointer;
extern volatile char rx_buff[];
extern volatile uint8_t bytes_to_parse;
extern int16_t cabin_position;

//CRC tabulka konstanty
const char crc_table[256] = {
	
};

//funkcia na strucny vypocet crc
char crc(char cielova_addr, char zdrojova_addr, char* data, size_t dlzka){
	char crc = 0;

	crc = crc_table[cielova_addr ^ crc];
	crc = crc_table[zdrojova_addr ^ crc];

	for(unsigned i=0; i<dlzka; i++)
		crc = crc_table[data[i] ^ crc];

	return crc;
}

//metoda poslanie spravy obsahujuca ciel, data
void protocol_sendMessage(char ciel, char *data, size_t data_dlzka){
	char buff[5 + data_dlzka];

	buff[0] = 0xA0;
	buff[1] = ciel;
	buff[2] = 0x00;
	buff[3] = data_dlzka;
	memcpy(buff+4, data, data_dlzka);
	buff[4 + data_dlzka] = crc(ciel, 0x00, data, data_dlzka);

	ack = ciel;


	uint8_t retry_cnt = RETRY_COUNT;
	while(retry_cnt--){
		lpsci_sendMessage(buff, data_dlzka+5);

		uint8_t timeout = 50;
		while(timeout-- && ack){
			if(rx_buff_read_pointer != rx_buff_write_pointer)
				analize_incoming_packet();

		}
	}
}

//metoda na poslanie spravy s potvrdzovacim acknolič
void send_ack(char ciel){
	char buff[5] = {0};

	buff[0] = 0xA1;
	buff[1] = ciel;
	buff[4] = crc(ciel, 0,  NULL, 0);

	lpsci_sendMessage(buff, 5);
}
//metoda na analizovanie prichadzajuceho paketu s pamete
void analize_incoming_packet(){
	uint8_t start = 255;
	char vytazenie;
	bytes_to_parse--;
	rx_buff_read_pointer++;
	rx_buff_read_pointer %= RX_RINGBUFF_SIZE;


	uint8_t pos = rx_buff_read_pointer;

	if((rx_buff[rx_buff_read_pointer + RX_RINGBUFF_SIZE - 4] & 0xFE) == 0xA0)
		start = rx_buff_read_pointer + RX_RINGBUFF_SIZE - 4;
	if((rx_buff[rx_buff_read_pointer + RX_RINGBUFF_SIZE - 5] & 0xFE) == 0xA0)
		start = rx_buff_read_pointer + RX_RINGBUFF_SIZE - 5;
	if((rx_buff[rx_buff_read_pointer + RX_RINGBUFF_SIZE - 6] & 0xFE) == 0xA0)
		start = rx_buff_read_pointer + RX_RINGBUFF_SIZE - 6;

	if(start == 255)	//ziaden start nenajdeny
		return;

	pos = (start + 1) % RX_RINGBUFF_SIZE;

	if(rx_buff[pos] != CONTROLLER_ADDR)	//kontrola cielovej adresy
		return;

	char zdrojova_addr = packet(2);
	char vytazenie_size = packet(3);

	vytazenie = 0;

	if(vytazenie_size == 0){		//sprava bez dat
		if(packet(4) != crc(CONTROLLER_ADDR, zdrojova_addr, NULL, 0))
			return;		//kontrolny sucet sa nezhoduje
	}
	else if(vytazenie_size == 1){		//sprava s 1B dat
		vytazenie = packet(4);
		if(packet(5) != crc(CONTROLLER_ADDR, zdrojova_addr, &vytazenie, 1))
			return;		//kontrolny sucet sa nezhoduje
	}
	else if(vytazenie_size == 2){		//sprava s 1B dat
		char sprava[2];

		sprava[0] = packet(4);
		sprava[1] = packet(5);

		if((rx_buff[start] == 0xA0) && (zdrojova_addr == MOTOR) && (packet(6) == crc(CONTROLLER_ADDR, zdrojova_addr, sprava, 2))){
			cabin_position = * (int16_t *)sprava;
			send_ack(zdrojova_addr);
		}  								//kontrolny sucet sa zhoduje

		return;
	}



	else
		return;
	if(rx_buff[start] == 0xA1 && zdrojova_addr == ack)	//prislo potvrdenie poslednej odoslanej spravy
		ack = 0;
	else if(rx_buff[start] == 0xA0){	//prisla nova sprava
		if(event_add(zdrojova_addr, vytazenie))
			send_ack(zdrojova_addr);
	}

}
